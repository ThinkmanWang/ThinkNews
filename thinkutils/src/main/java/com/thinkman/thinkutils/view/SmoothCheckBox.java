...
public class SmoothCheckBox extends View implements Checkable {
    ...
    private void init(AttributeSet attrs) {
        ...
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.SmoothCheckBox_scb_stroke_width, DisplayUtil.dip2px(getContext(), 0));
        ta.recycle();
    ...
    public void showChecked(boolean checked) {
        mChecked = checked;
        reset();
        invalidate();
    }

    public void showChecked(boolean checked, boolean animate) {
        if (animate) {
            mTickDrawing = false;
            mChecked = checked;
            mDrewDistance = 0f;
